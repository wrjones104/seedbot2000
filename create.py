...
def cr_search(paint, c_rating, fixed_flags = '', range_flags = ''):
    ...
    ### CONTROLS ###-
    cr_timeout = 2000  # Maximum number of loops
    ...
    while counter < cr_timeout:
        ...
        options = [k for k in range_list[this_flag]]
        if verbose:
            print(' ')
        ...
        if verbose:
            ...
            for jjj in range(len(op_weight)):
                print('     ', options[jjj], ': d=', round(dist[jjj], 3), ', CR = ', round(op_CR[jjj],2),', weight =',  round(op_weight[jjj],5))
        ...
        if range_list[this_flag] == [True, False] and ymin == np.min(dist):
            # Harsh adjustment
