...
def get_cr(fl):
    ...
    # RANDOM EXCLUDED SKILLS
    if '-rec1' in cr_flags:
        rec1v = str(int(sf1('-rec1 ')))
        if len(rec1v) == 1:
            rec1v = ''.join(["0", rec1v])
        recvar1 = 2 - skills[rec1v]
    else:
        recvar1 = 0

    if '-rec2' in cr_flags:
        rec2v = str(int(sf1('-rec2 ')))
        if len(rec2v) == 1:
            rec2v = ''.join(["0", rec2v])
        recvar2 = 2 - skills[rec2v]
    else:
        recvar2 = 0

    if '-rec3' in cr_flags:
        rec3v = str(int(sf1('-rec3 ')))
        if len(rec3v) == 1:
            rec3v = ''.join(["0", rec3v])
        recvar3 = 2 - skills[rec3v]
    else:
        recvar3 = 0

    if '-rec4' in cr_flags:
        rec4v = str(int(sf1('-rec4 ')))
        if len(rec4v) == 1:
            rec4v = ''.join(["0", rec4v])
        recvar4 = 2 - skills[rec4v]
    else:
        recvar4 = 0

    if '-rec5' in cr_flags:
        rec5v = str(int(sf1('-rec5 ')))
        if len(rec5v) == 1:
            rec5v = ''.join(["0", rec5v])
        recvar5 = 2 - skills[rec5v]
    else:
        recvar5 = 0
    ...
    return cr_flags, round(float(cr), 2)
